...
<body class="h-screen flex items-center justify-center overflow-hidden relative p-4">
    ...
    <div class="fixed top-8 left-1/2 transform -translate-x-1/2 z-[100] w-full max-w-2xl px-4 space-y-4">
        ...
        <div class="bg-emerald-500 text-white p-6 rounded-[2rem] shadow-2xl flex items-center space-x-4 animate-[bounce_1s_ease-in-out_1] border-4 border-white">
            <div class="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center text-2xl shrink-0">
                <i class="fa-solid fa-check"></i>
            </div>
            <div>
                <p class="font-black uppercase tracking-widest text-xs">¡Turno Generado exitosamente!</p>
                <p class="text-lg font-poppins font-black">{{ session('success') }}</p>
            </div>
        </div>
        @endif

        @if(session('warning'))
        <div class="bg-amber-500 text-white p-6 rounded-[2rem] shadow-2xl flex items-center space-x-4 animate-pulse border-4 border-white">
            <div class="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center text-2xl shrink-0">
                <i class="fa-solid fa-triangle-exclamation"></i>
            </div>
            <div>
                <p class="font-black uppercase tracking-widest text-xs">Advertencia de Registro</p>
                <p class="text-xs font-bold leading-tight mt-1">{{ session('warning') }}</p>
            </div>
        </div>
        @endif
        ...
        <div class="bg-rose-500 text-white p-6 rounded-[2rem] shadow-2xl flex items-center space-x-4 border-4 border-white">
            ...
            <div>
                <p class="font-black uppercase tracking-widest text-xs">Atención</p>
    ...
    <div class="w-full max-w-4xl mx-auto main-card rounded-[2.5rem] flex flex-col items-center relative shadow-2xl overflow-y-auto" style="max-height: 96vh;">
        ...
        <form id="kioskForm" action="{{ route('turnos.store') }}" method="POST" onsubmit="return validateForm();" class="flex-1 w-full flex flex-col items-center justify-center min-h-0">
            ...
            <div id="step1" class="step-content active w-full flex-col items-center justify-center text-center p-10 space-y-8">
                ...
                <div class="w-24 h-24 bg-white rounded-[1.5rem] shadow-xl flex items-center justify-center p-4 relative mx-auto">
                    <div class="absolute -inset-3 bg-white/20 blur-xl rounded-[2rem] -z-10"></div>
                    <img src="{{ asset('images/logoSena.png') }}" class="w-full h-auto" alt="Logo SENA">
                </div>

                <div class="space-y-3">
                    <h1 id="welcomeTitle" class="text-5xl font-black text-slate-800 tracking-tighter leading-tight uppercase">
                        Bienvenido al <br><span class="text-sena-500">Centro de Atención</span>
                    </h1>
                ...
                <div class="flex items-center justify-center gap-3 flex-wrap pt-2">
                    <a href="{{ route('pantalla.index') }}" class="flex items-center gap-2 px-4 py-2 bg-white/60 hover:bg-white text-slate-600 text-xs font-bold rounded-xl border border-white/50 transition-all">
                        <i class="fa-solid fa-display text-sena-500"></i> Pantalla
                    </a>
                    <a href="{{ route('asesor.login') }}" class="flex items-center gap-2 px-4 py-2 bg-white/60 hover:bg-white text-slate-600 text-xs font-bold rounded-xl border border-white/50 transition-all">
                        <i class="fa-solid fa-headset text-sena-500"></i> Asesor
                    </a>
                    <a href="{{ route('coordinador.login') }}" class="flex items-center gap-2 px-4 py-2 bg-white/60 hover:bg-white text-slate-600 text-xs font-bold rounded-xl border border-white/50 transition-all">
            ...
            <div id="step2" class="step-content w-full flex-col items-center justify-center p-10 md:p-14 space-y-8">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-5 w-full max-w-4xl">
                    ...
                    <div class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col items-center text-center space-y-3">
                        <div class="w-12 h-12 {{ $t['color'] }} rounded-2xl flex items-center justify-center text-xl">
                    ...
                    <label class="flex items-center gap-6 cursor-pointer group" for="termsCheck">
                        <div class="relative shrink-0">
                            <input type="checkbox" id="termsCheck" onchange="toggleBtn(this)"
                                class="peer appearance-none w-14 h-14 rounded-2xl bg-gray-100 border-2 border-gray-200 checked:bg-sena-500 checked:border-sena-500 transition-all cursor-pointer">
                        ...
                        <span class="text-xl font-black text-slate-700 leading-snug">
                ...
                <div class="flex gap-4 w-full max-w-3xl">
                    <button type="button" onclick="nextStep(1)"
                        class="px-10 py-5 bg-white border border-gray-200 rounded-2xl text-gray-400 font-black uppercase tracking-widest text-sm hover:bg-gray-50 active:scale-95 transition-all">
                        Volver
                    </button>
                    <button type="button" id="nextBtn" onclick="nextStep(3)" disabled
                        class="flex-1 py-5 rounded-2xl bg-gray-200 text-gray-400 font-black flex items-center justify-center gap-3 cursor-not-allowed opacity-50 transition-all text-sm uppercase tracking-widest">
            ...
            <div id="step3" class="step-content w-full flex-col items-center justify-center p-6 md:p-8 space-y-6">
                ...
                <div class="text-center space-y-2">
                    <h2 class="text-3xl font-poppins font-black text-[#1e293b] tracking-tight leading-tight">
                        ¿Cuál es su <span class="text-sena-500">categoría</span> de usuario?
                    </h2>
                    <p class="text-sm font-medium text-slate-500">
                        Seleccione la opción que mejor describa su condición.
                    </p>
                </div>
                ...
                <div onclick="selectType('{{ $p['id'] }}')"
                    class="bg-white p-5 rounded-2xl border {{ $p['border'] }} shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer group flex flex-col items-center text-center space-y-3">
                    
                    <div class="w-12 h-12 {{ $p['color'] }} rounded-xl flex items-center justify-center text-xl group-hover:scale-110 transition-transform">
                        <i class="fa-solid {{ $p['icon'] }}"></i>
                    </div>

                    <h4 class="text-base font-black text-slate-800">{{ $p['title'] }}</h4>

                    <div class="px-3 py-1 rounded-full {{ $p['color'] }}">
                        <span class="text-[8px] font-black uppercase tracking-wider">{{ $p['badge'] }}</span>
                    </div>
                ...
                <div class="w-full bg-slate-50 p-4 rounded-2xl border border-gray-100 text-center">
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        Al seleccionar su categoría, el sistema le asignará un turno. Si necesita asistencia presione <span class="text-sena-500">Ayuda</span>.
                    </p>
                </div>
            </div>
            <!-- STEP 3.5: SERVICIO Y TIPO DE ATENCIÓN (NUEVO) -->
            <div id="step3_5" class="step-content w-full flex-col items-center justify-center p-8 md:p-14 lg:p-16 space-y-12">
                <div class="text-center space-y-4 max-w-4xl mx-auto">
                    <h2 class="text-5xl font-poppins font-black text-[#1e293b] tracking-tighter leading-none">
                        DETALLES DE LA <span class="text-sena-500">VISITA</span>
                    </h2>
                </div>
            ...
            <div id="step4" class="step-content w-full flex-col items-center justify-center p-6 md:p-10 space-y-6">
                ...
                <!-- Tipo de documento en fila -->
                <div class="flex gap-3 w-full max-w-2xl">
                    @foreach(['CC' => 'Cédula', 'CE' => 'Extranjería', 'TI' => 'Tarjeta Identidad'] as $val => $label)
                    <button type="button" onclick="setDocType('{{ $val }}', this); playKey();"
                        class="doc-tab-btn flex-1 py-4 rounded-2xl border-2 font-black text-sm uppercase tracking-widest transition-all
                        {{ $val == 'CC' ? 'border-sena-500 bg-sena-50 text-sena-500 shadow-md' : 'border-gray-100 text-slate-400 hover:border-gray-200' }}">
                        {{ $label }}
                    </button>
                    @endforeach
                </div>
                ...
                <div class="grid grid-cols-3 gap-3 w-full max-w-2xl">
                    @for($i=1;$i<=9;$i++)
                    <button type="button" onclick="pressNum('{{ $i }}'); playKey();"
                        class="h-20 bg-slate-50 hover:bg-white border-2 border-transparent hover:border-sena-100 rounded-2xl flex items-center justify-center text-3xl font-black text-slate-700 shadow-sm hover:shadow-md transition-all active:scale-95">{{ $i }}</button>
                    @endfor
                    <button type="button" onclick="clearNum(); playKey();"
                        class="h-20 bg-rose-50 hover:bg-rose-100 rounded-2xl flex items-center justify-center text-2xl text-rose-500 shadow-sm transition-all active:scale-95">
                        <i class="fa-solid fa-trash-can"></i>
                    </button>
                    <button type="button" onclick="pressNum('0'); playKey();"
                        class="h-20 bg-slate-50 hover:bg-white border-2 border-transparent hover:border-sena-100 rounded-2xl flex items-center justify-center text-3xl font-black text-slate-700 shadow-sm hover:shadow-md transition-all active:scale-95">0</button>
                    <button type="button" onclick="backspace(); playKey();"
                        class="h-20 bg-slate-50 hover:bg-white rounded-2xl flex items-center justify-center text-2xl text-slate-400 shadow-sm transition-all active:scale-95">
                ...
                <button type="button" onclick="validateDoc(); playKey();"
                    class="w-full max-w-2xl py-6 bg-sena-orange text-white font-black text-xl rounded-[2rem] shadow-xl hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-4 uppercase tracking-widest">
                    <span>CONTINUAR</span>
                    <i class="fa-solid fa-arrow-right"></i>
                </button>
            </div>
            ...
            <div id="step5" class="step-content w-full flex-col items-center justify-center p-6 md:p-8 space-y-5">
                ...
                <div class="grid grid-cols-3 gap-3 w-full max-w-2xl">
                    @for($i=1;$i<=9;$i++)
                    <button type="button" onclick="pressPhone('{{ $i }}'); playKey();"
                        class="h-16 bg-white hover:bg-slate-50 border-2 border-transparent hover:border-sena-100 rounded-2xl flex items-center justify-center text-2xl font-black text-slate-700 shadow-sm hover:shadow-md transition-all active:scale-95">{{ $i }}</button>
                    @endfor
                    <button type="button" onclick="phoneNumber=''; updatePhoneDisplay(); playKey();"
                        class="h-16 bg-white rounded-2xl flex items-center justify-center text-xl text-slate-300 shadow-sm hover:text-rose-500 transition-colors">
                        <i class="fa-solid fa-rotate-right"></i>
                    </button>
                    <button type="button" onclick="pressPhone('0'); playKey();"
                        class="h-16 bg-white hover:bg-slate-50 border-2 border-transparent hover:border-sena-100 rounded-2xl flex items-center justify-center text-2xl font-black text-slate-700 shadow-sm hover:shadow-md transition-all active:scale-95">0</button>
                    <button type="button" onclick="backspacePhone(); playKey();"
                        class="h-16 bg-white rounded-2xl flex items-center justify-center text-xl text-slate-300 shadow-sm hover:text-amber-500 transition-colors">
                        <i class="fa-solid fa-delete-left"></i>
                    </button>
                </div>

                <!-- Botón continuar -->
                <button type="button" onclick="nextStep(6); playKey();"
                    class="w-full max-w-2xl py-5 bg-sena-orange text-white font-black text-lg rounded-[2rem] shadow-xl hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-3 uppercase tracking-widest">
                    <span>CONTINUAR</span>
            ...
            <div id="step6" class="step-content w-full flex-col items-center justify-center p-6 md:p-8 space-y-6">
                ...
                <div class="grid grid-cols-4 gap-3 w-full">
                    @php 
                    $methods = [
                        ['id'=>'SMS',      'icon'=>'fa-comment-sms', 'title'=>'SMS',        'fab'=>false],
                        ['id'=>'WhatsApp', 'icon'=>'fa-whatsapp',    'title'=>'WhatsApp',   'fab'=>true],
                        ['id'=>'Email',    'icon'=>'fa-envelope',    'title'=>'Email',      'fab'=>false],
                        ['id'=>'QR',       'icon'=>'fa-qrcode',      'title'=>'Código QR',  'fab'=>false],
                    ];
                    @endphp
                    @foreach($methods as $m)
                    <button type="button" onclick="selectChannel('{{ $m['id'] }}', this); playKey();"
                        class="receive-card bg-white p-5 rounded-2xl border-2 border-gray-100 transition-all duration-300 flex flex-col items-center gap-3 hover:border-sena-500 hover:shadow-md active:scale-95">
                ...
                <!-- Panel dinámico según canal seleccionado -->
                <div id="channel-panel" class="w-full hidden">
                    ...
                    <div id="panel-Email" class="channel-content hidden bg-blue-50 border border-blue-200 rounded-2xl p-5 space-y-3">
                        ...
                        <input type="email" id="email-input" name="pers_email" placeholder="ejemplo@correo.com"
                            class="w-full bg-white border-2 border-blue-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 focus:outline-none focus:border-sena-500 transition-all text-center">
                ...
                <div class="flex gap-3 w-full">
                    <button type="button" onclick="nextStep(5); playKey();"
                        class="px-8 py-4 bg-white border border-gray-200 rounded-2xl text-slate-400 font-black uppercase tracking-widest text-xs hover:bg-gray-50 active:scale-95 transition-all flex items-center gap-2">
                        <i class="fa-solid fa-arrow-left"></i> VOLVER
                    </button>
                    <button type="submit" onclick="playKey();"
                        class="flex-1 py-4 bg-gradient-to-r from-sena-500 to-sena-orange text-white font-black text-sm rounded-2xl shadow-lg hover:-translate-y-0.5 transition-all flex items-center justify-center gap-3 uppercase tracking-widest">
    ...
    <div id="successModal" class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
        <div class="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row border border-white/20">
            ...
            <div class="lg:w-2/5 bg-sena-500 p-10 flex flex-col items-center justify-center text-center space-y-6 relative overflow-hidden">
                ...
                <div class="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-xl relative z-10">
                    <i class="fa-solid fa-check text-4xl text-sena-500"></i>
                </div>
            ...
            <div class="lg:w-3/5 p-8 flex flex-col items-center justify-center space-y-5">
                ...
                <div class="bg-slate-50 border-4 border-slate-100 rounded-2xl px-8 py-6 w-full text-center shadow-inner">
                    <div class="text-[4.5rem] font-poppins font-black text-sena-500 tracking-tight leading-none whitespace-nowrap">
                        {{ $turnoNumero }}
                    </div>
                </div>

                <!-- QR del turno (siempre visible) -->
                <div class="flex flex-col items-center space-y-2">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Escanee para guardar su turno</p>
                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=140x140&data={{ urlencode('SENA APE - Turno: ' . $turnoNumero) }}"
                         alt="QR Turno {{ $turnoNumero }}"
                         class="w-36 h-36 rounded-2xl border-4 border-slate-100 shadow-sm">
                </div>
                ...
                <button onclick="closeModal()" class="w-full py-4 rounded-2xl bg-sena-orange text-white font-black text-sm uppercase tracking-widest shadow-lg active:scale-95 transition-all">
                    FINALIZAR <i class="fa-solid fa-chevron-right ml-2"></i>
                </button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function selectType(type) { 
            ...
            // tur_tipo se sigue enviando por compatibilidad (en TurnoController se sobreescribe)
        ...
        function setService(val, btn) {
            ...
            document.querySelectorAll('.service-btn').forEach(b => {
                b.classList.remove('border-sena-500', 'bg-sena-50', 'text-sena-500');
                b.querySelector('i').classList.add('opacity-0');
            });
            btn.classList.add('border-sena-500', 'bg-sena-50', 'text-sena-500');
        ...
        function setAttentionType(val, btn) {
            ...
            document.querySelectorAll('.attention-btn').forEach(b => {
                b.classList.remove('border-sena-500', 'bg-sena-50', 'text-sena-500');
                b.querySelector('i').classList.add('opacity-0');
            });
            btn.classList.add('border-sena-500', 'bg-sena-50', 'text-sena-500');
        ...
        function setDocType(type, btn) {
            ...
            document.querySelectorAll('.doc-tab-btn').forEach(b => {
                b.classList.remove('border-sena-500', 'bg-sena-50', 'text-sena-500', 'shadow-md');
                b.classList.add('border-gray-100', 'text-slate-400');
            });
            btn.classList.add('border-sena-500', 'bg-sena-50', 'text-sena-500', 'shadow-md');
            btn.classList.remove('border-gray-100', 'text-slate-400');
        }
        ...
        function updateDocDisplay() { 
            ...
            d.style.letterSpacing = docNumber ? "0.15em" : "0.1em";
        ...
        function selectChannel(method, btn) {
            ...
            document.querySelectorAll('.receive-card').forEach(c => {
                c.classList.remove('border-sena-500', 'bg-sena-50');
                c.querySelector('div').classList.remove('bg-sena-500', 'text-white');
                c.querySelector('div').classList.add('bg-gray-50', 'text-gray-400');
            });
            btn.classList.add('border-sena-500', 'bg-sena-50');
            btn.querySelector('div').classList.add('bg-sena-500', 'text-white');
            btn.querySelector('div').classList.remove('bg-gray-50', 'text-gray-400');
            ...
            if (method === 'WhatsApp') {
                document.getElementById('wa-number').textContent = '+57 ' + phone;
            }
            if (method === 'SMS') {
                document.getElementById('sms-number').textContent = '+57 ' + phone;
            }
            if (method === 'QR') {
                // Generar QR usando API pública (no requiere instalación)
                const turnoInfo = encodeURIComponent('SENA APE - Doc: ' + doc + ' | Tel: ' + phone);
                const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=' + turnoInfo;
        ...
        function validateForm() {
            ...
            // Sincronizar email si el canal es Email
