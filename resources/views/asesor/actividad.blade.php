...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h2 class="text-2xl font-black text-gray-900 leading-tight">Registro de Actividad</h2>
        <p class="text-gray-500 text-[11px] font-medium mt-0.5">Bitácora completa y detallada de tus atenciones realizadas.</p>
    </div>
    <div class="flex items-center space-x-3 bg-white px-4 py-2 rounded-xl shadow-sm border border-gray-100 italic text-[9px] font-black tracking-widest cursor-pointer group hover:bg-emerald-50 hover:border-emerald-200 transition-all" onclick="toggleRealTime()" id="realtimeToggle">
        <i class="fa-solid fa-clock-rotate-left text-gray-400 group-hover:text-emerald-500 transition-colors" id="rtIcon"></i>
        <span class="text-gray-400 group-hover:text-emerald-600 transition-colors" id="rtText">Historial en Vivo: Inactivo</span>
    </div>
</div>
...
<div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-6 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
        <div class="flex items-center space-x-3">
            <div class="px-3 py-1.5 bg-sena-500 text-white rounded-lg text-[9px] font-black uppercase tracking-widest">Hoy</div>
            <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ now()->format('d M, Y') }}</div>
        </div>
        <form id="filterForm" action="{{ route('asesor.actividad') }}" method="GET" class="flex items-center space-x-2">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar..." class="px-3 py-2 bg-white border border-gray-100 rounded-lg text-[11px] font-bold text-gray-700 outline-none focus:ring-2 focus:ring-sena-500 transition-all w-36 placeholder-gray-300">
            <select name="estado" class="px-3 py-2 bg-white border border-gray-100 rounded-lg text-[11px] font-bold text-gray-700 outline-none cursor-pointer focus:ring-2 focus:ring-sena-500">
            ...
            <button type="submit" class="h-8 px-3 flex items-center justify-center bg-sena-50 text-sena-600 font-bold border border-sena-100 rounded-lg hover:bg-sena-500 hover:text-white transition-all shadow-sm">
                <i class="fa-solid fa-filter text-[10px] mr-1.5"></i>
                <span class="text-[9px] uppercase tracking-widest">Filtrar</span>
            </button>
            <a href="{{ route('asesor.actividad', array_merge(request()->all(), ['export' => 'excel'])) }}" class="h-8 px-4 flex items-center justify-center bg-emerald-600 text-white font-bold border border-emerald-700 rounded-lg hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/20 active:scale-95 group">
                <i class="fa-solid fa-file-excel text-[10px] mr-1.5 group-hover:-translate-y-0.5 transition-transform"></i>
                <span class="text-[9px] uppercase tracking-widest">Excel</span>
            </a>
        </form>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-50">
                    <th class="px-6 py-4">Turno / ID</th>
                    <th class="px-6 py-4">Información del Ciudadano</th>
                    <th class="px-6 py-4">Intervalo de Atención</th>
                ...
                <tr class="hover:bg-gray-50/50 transition-colors group cursor-default">
                    <td class="px-6 py-5">
                        <div class="flex flex-col">
                            <span class="text-xs font-black text-gray-900 group-hover:text-sena-500 transition-colors">{{ $atn->turno->tur_numero }}</span>
                            <span class="text-[8px] font-bold text-gray-400 uppercase tracking-tighter mt-0.5">ID #{{ str_pad($atn->atnc_id ?? rand(100, 999), 5, '0', STR_PAD_LEFT) }}</span>
                        </div>
                    </td>
                    <td class="px-10 py-8">
